...
Recommend some music for me to listen to while I grade your final Exams on Sunday May 4.

# Problem 1

In statistics, we observe data $X_1,\,X_2,\,...,\,X_n\overset{\text{iid}}{\sim} P_0$ from an unknown probability distribution, and we try to use the data to estimate some feature $\theta_0=\theta(P_0)$ of the unknown distribution. Let's assume that $\theta_0\in\mathbb{R}$ is a scalar summary, like the mean, median, variance, etc. We estimate $\theta_0$ by constructing an **estimator**:
...
$$
\text{MSE}=E[(\hat{\theta}_n-\theta_0)^2].
$$
...
$$
\begin{aligned}
E[(\hat{\theta}_n-\theta_0)^2] &= \left[E(\hat{\theta}_n) - \theta_0\right]^2 + \text{var}(\hat{\theta}_n)\\
...
Let $\mu_0=E(X_1)$ and $\sigma_0^2=\text{var}(X_1)$ be the true but unknown mean and variance of the data, and let's focus on the task of estimating the mean. So far, we have used the sample average $\bar{X}_n=\sum_{i=1}^nX_i/n$ to do this, but let's consider a new estimator: $$
...
Imagine that prior research says that the mean is probably $g=1.6$, and you are conducting a new study with $n=10$. Then one night Zeus comes to you in a dream and whispers in your ear that, in fact, the true values are $\mu_0=10$ and $\sigma_0^2=1$. Yeesh, with friends like Zeus, who needs statistics!
...
Imagine you have iid data $X_1$, $X_2$, ..., $X_n$ from a distribution with the following density, where $\theta>0$ is a parameter:
...
X_1,\,X_2,\,...,\,X_n\,|\,\lambda&\sim\text{Exponential}(\lambda) && \text{(data model)}.
\end{aligned}
$$

$\lambda \sim\text{Gamma}(\alpha_0,\,\beta_0)$ is my prior distribution for the unknown parameter, and $\alpha_0,\,\beta_0>0$ are *hyperparameters* that I will tune in order to encode the prior knowledge about $\lambda$ that my grandmother described. I chose the Gamma family because it is familiar to me, and I know that $\lambda$ is a continuous numerical quantity that must be positive.
